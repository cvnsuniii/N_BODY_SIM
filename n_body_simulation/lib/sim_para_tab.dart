...
class Simparastatetab extends State<Sim_para_tab> {
  ...
  void mist(){
    ...
    three.Vector3 center = three.Vector3(0,0,0);
    List<List<double>> L=[];
    ...
    for (int k=0; k<(simtimes*1000/timestep).toInt();k++){
      ...
      for (int m=0; m<simdata[simtitle]!.length.toInt();m++){
        ...
        for (int d=0; d<simdata[simtitle]!.length.toInt();d++){
          ...
          if (r/2>radius){
            center=three.Vector3(l[d][0],l[d][1],l[d][2]).add(three.Vector3(l[m][0],l[m][1],l[m][2]));
            center=center.divideScalar(2);
            radius=(r/2).toDouble();
          }
          if (r!=0){
    ...
    for ( int m=0; m<pointgrp.length; m++){
      for (int d=((m~/simdata[simtitle]!.length.toInt()).toInt()+1)*simdata[simtitle]!.length.toInt()+1; d<pointgrp.length; d++){
        num r=pow((pow((pointgrp[d].x-pointgrp[m].x),2)+pow((pointgrp[d].y-pointgrp[m].y),2)+pow((pointgrp[d].z-pointgrp[m].z),2)),0.5);
        if (r/2>radius){
          center=pointgrp[m].add(pointgrp[d]);
          center=center.divideScalar(2);
          radius=(r/2).toDouble();
        }
      }
        ...
        if(snapshot.connectionState == ConnectionState.done){
          ...
          return Scaffold(
            appBar:AppBar(
              ...
              actions:[if(checked) FloatingActionButton(heroTag: null,backgroundColor:Colors.blueAccent,onPressed:()async{final animation=calculate22(simdata, timestep, sp1, simtimes,); await Navigator.of(context).push(MaterialPageRoute(builder: (context){return Sim(title:simtitle,simulation:animation,user:widget.user,timestep:timestep,speed:sp1,simtime:simtimes,centroids:centroids,simdata:simdata,radius:radius);}));setState((){});},child:Text("Simulate",style:TextStyle(color: Colors.white)))]
            ),
            body:Center(
              child: Container(
                      ...
                      child:Column(children: [  
                        ...
                        Row(spacing:20,children:[
                          ...
                          if(checked) FloatingActionButton(heroTag:null,backgroundColor:Colors.blueAccent,onPressed:()async{
                            if(chkdist()){final animation=calculate22(simdata, timestep, sp1, simtimes);await Navigator.of(context).push(MaterialPageRoute(builder: (context){return Sim(title:simtitle,simulation:animation,user:widget.user,timestep:timestep,speed:sp1,simtime:simtimes,centroids:centroids,simdata:simdata,radius:radius);})); setState((){});}
